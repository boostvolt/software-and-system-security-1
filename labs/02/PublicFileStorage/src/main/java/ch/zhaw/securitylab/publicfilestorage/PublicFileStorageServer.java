...
public class PublicFileStorageServer {
    ...
    private boolean validateCommand(String options) {
    ...
    private String readLineMaxChar(Reader reader, int max) throws IOException {
        ...
        while ((ch = reader.read()) != -1) {
            ...
            if (count >= max) {
                throw new IOException("Line exceeds maximum length of " + max + " characters");
            }
        ...
        // If nothing was read and EOF reached, return null to indicate end-of-stream
        if (ch == -1 && sb.isEmpty()) {
            return null;
        }
